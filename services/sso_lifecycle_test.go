	...
	Describe("For service clients (provided in catalog)", func() {
		Context("When a service broker is created with a service dashboard client", func() {
			It("can perform an operation on a user's behalf using sso", func() {
				...
				serviceInstanceDetails := cf.Cf("service", instanceName, "--guid").Wait(DEFAULT_TIMEOUT)
				Expect(serviceInstanceDetails).To(Exit(0), "failed creating service")
				serviceInstanceGuid := strings.Trim(string(serviceInstanceDetails.Out.Contents()), "\n")
